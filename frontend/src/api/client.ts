...
type ProblemImportUploadOptions = {
  onProgress?: (percent: number) => void;
  onProcessing?: () => void;
};
...
export function uploadProblemsFile(file: File, options: ProblemImportUploadOptions = {}): Promise<any> {
  return new Promise((resolve, reject) => {
    const formData = new FormData();
    formData.append('file', file);

    const request = new XMLHttpRequest();
    request.open('POST', `${API_BASE}/problems/upload`);
    request.setRequestHeader('x-user-id', getActiveUserId());
    request.upload.onprogress = event => {
      if (event.lengthComputable) {
        options.onProgress?.(Math.min(100, Math.round((event.loaded / event.total) * 100)));
    ...
    request.upload.onload = () => options.onProcessing?.();
    request.onerror = () => reject(new Error('The problem upload failed. Check the server connection and try again.'));
    request.onabort = () => reject(new Error('The import was cancelled.'));
    request.onload = () => {
      let data: any = null;
      try {
      ...
      const fallback = request.status === 413
        ? 'This dataset exceeds the server upload size limit.'
        : `Problem import failed (HTTP ${request.status || 'unknown'}).`;
      reject(new Error(data?.error || fallback));
    };
    request.send(formData);
  });
...
export const api = {
  ...
  importProblems: (data: any) =>
    apiRequest('/problems/upload', { method: 'POST', body: JSON.stringify(data) }),
  importProblemsFile: (file: File, options?: ProblemImportUploadOptions) =>
    uploadProblemsFile(file, options),
